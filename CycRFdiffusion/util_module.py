...
def get_seqsep(idx):
    ...
    # cyc_offset = np.abs(ij[:, None, :, None] - ij[None, :, None, :]).min((2, 3))
    # seqsep_pep = np.sign(c_offset) * cyc_offset
    # # offset = offset.at[-size:, -size:].set(r_offset)
    # seqsep_pep = torch.tensor(seqsep_pep, device='cuda:0')
    # seqsep_pep = torch.abs(seqsep_pep)
    print(seqsep_pep)
